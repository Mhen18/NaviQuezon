...
///{@template EstablishmentVisitedScreen}
/// Screen for the establishment that the user visited.
///{@endtemplate}
class EstablishmentVisitedScreen extends StatefulWidget {
  ///{@macro EstablishmentVisitedScreen}
  const EstablishmentVisitedScreen({super.key});

  static const route = rEstablishmentVisited;

  @override
  State<EstablishmentVisitedScreen> createState() => _ScreenState();
}

class _ScreenState extends State<EstablishmentVisitedScreen> {
                  ...
                  builder: (context, estaState) {
                    ...
                    if (estaState is CubitStateSuccess) {
                      ...
                      for (final esta in list) {
                        ...
                        final estaSurveyList = esta.survey ?? [];

                        if (estaSurveyList.isNotEmpty) {
                          for (final survey in estaSurveyList) {
                            if (survey.userId == profileId) {
                      ...
                      if (estaList.isEmpty) {
                        return const ErrorStateContainer(
                          message: 'No visited establishment found',
                        );
                      }
